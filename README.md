...
- 临时或永久修改名称、时间、地点、类型和备注；选择“自定义类型”后可自由输入并保留为常用选项
...
计划类型不限于内置的“课程、六级、计算机、课程复习、自主学习、休息”。新增或修改计划时，在“类型”中选择“＋ 自定义类型”，App 会显示独立文本框并在 iPhone 上尝试自动唤起键盘。可输入“社团、兼职、值班、约会”等任意类型（最多 30 个字符）；保存后会成为本机常用选项，并出现在日历导出筛选中。
